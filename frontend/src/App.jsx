...
function MapPage({ localities, selected, setSelected, risk, loading, error, onRefresh }) {
  const [panelOpen, setPanelOpen] = useState(Boolean(selected));
  useEffect(() => { setPanelOpen(Boolean(selected)); }, [selected]);
  ...
  return (
    <main className="mapPage">
      ...
      <div className="mapOverlay mapTitle">
        ...
        <p>Click a locality to inspect its current risk and forecast.</p>
      </div>
        ...
        {!loading && !error && risk && (
          <>
            <div className={`riskHero ${riskClass}`}>
              ...
              <div><small>Next 24-hour flood risk</small><strong>{risk.next_24h?.risk_band || 'Unknown'}</strong></div>
              <b>{Math.round((risk.next_24h?.probability || 0) * 100)}%</b>
            </div>
            <div className="stats">
              <Stat label="Current risk" value={risk.current?.risk_band || '—'} />
              <Stat label="Forecast rainfall" value={`${risk.next_24h?.forecast_rainfall_mm ?? 0} mm`} />
              <Stat label="Last 7 days" value={`${risk.context?.rainfall_last_7d_mm ?? 0} mm`} />
              <Stat label="Last 30 days" value={`${risk.context?.rainfall_last_30d_mm ?? 0} mm`} />
            </div>
            <div className="chartCard compactChart">
              <div className="cardTitle">Next 7 Days · Rainfall & Risk</div>
              ...
              <div className="forecastRows">
                {chart7.map(d => <div className="forecastRow" key={d.date}><span>{d.date}</span><b>{d.rainfall_mm} mm</b><em className={String(d.risk_band).toLowerCase()}>{d.risk_band}</em></div>)}
              </div>
            </div>
...
function MapResize() {
  const map = useMap();
  useEffect(() => { const timer = setTimeout(() => map.invalidateSize(), 50); return () => clearTimeout(timer); }, [map]);
  return null;
}
...
function LocalitiesPage({ localities, onOpen }) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    getJson(`${API}/flood-risk-all`).then(data => {
      if (!active) return;
      const next = {};
      (data?.results || []).forEach(item => { if (item.ok) next[item.name] = item.data; });
      setPredictions(next);
    }).catch(err => active && setMessage(err.message)).finally(() => active && setLoading(false));
    return () => { active = false; };
  }, [localities]);
  ...
  return <main className="page"><div className="eyebrow">LOCALITY ANALYSIS</div><h1>All Project Localities</h1><p className="subtitle">Live flood-risk and rainfall prediction for every configured locality.</p>
    <div className="toolbar localityToolbar"><input value={query} placeholder="Search locality…" onChange={e => setQuery(e.target.value)} /><select value={sort} onChange={e => changeSort(e.target.value)}><option value="name">Sort: Locality</option><option value="risk">Sort: Risk probability</option><option value="rainfall">Sort: Forecast rainfall</option></select><span className="sortHint">Click the sort selector again to reverse order</span></div>
    {message && <div className="notice">{message}</div>}
    <div className="table"><div className="thead localityGrid"><span>LOCALITY</span><span>24H RISK</span><span>PROBABILITY</span><span>RAINFALL</span><span>ACTION</span></div>
      {loading && <div className="tableMessage">Loading all locality predictions…</div>}
      {!loading && rows.map(x => { const p = predictions[x.name]; return <button className="tr localityGrid" key={x.name} onClick={() => onOpen(x.name)}><span><b>{x.name}</b><small>{x.latitude.toFixed(4)}, {x.longitude.toFixed(4)}</small></span><span className={String(p?.next_24h?.risk_band || '—').toLowerCase()}>{p?.next_24h?.risk_band || '—'}</span><span>{p ? `${Math.round((p.next_24h.probability || 0) * 100)}%` : '—'}</span><span>{p ? `${p.next_24h.forecast_rainfall_mm} mm` : '—'}</span><span>View →</span></button>; })}
    </div></main>
}
...
function RainfallPage({ localities, selected, setSelected }) {
  ...
  async function generate() { if (!selected) return; setLoading(true); setError(''); try { setData(await getJson(`${API}/rainfall-forecast/locality/${encodeURIComponent(selected)}?months=${months}`)); } catch (err) { setError(err.message); } finally { setLoading(false); } }
  useEffect(() => { if (selected) generate(); }, [selected]);

  const chart = useMemo(() => data?.forecast || [], [data]);
  return <main className="page"><div className="eyebrow">RAINFALL FORECASTING</div><h1>Rainfall Prediction</h1><p className="subtitle">Locality-specific SARIMA rainfall forecast with a clear trend graph.</p>
    <div className="controlCard"><div><label>LOCALITY</label><select value={selected} onChange={e => setSelected(e.target.value)}>{localities.map(x => <option key={x.name} value={x.name}>{x.name}</option>)}</select></div><div><label>HORIZON</label><select value={months} onChange={e => setMonths(Number(e.target.value))}><option value={12}>12 months</option><option value={24}>24 months</option><option value={36}>36 months</option></select></div><button className="primary" onClick={generate} disabled={loading}>{loading ? 'Generating…' : 'Generate Forecast'}</button></div>
    {error && <div className="errorBox">{error}</div>}
    {data?.status === 'insufficient_historical_data' && <div className="notice">{data.message}. This locality has {data.observed_months} observed months; at least 24 are required.</div>}
    {data?.status === 'model_fit_failed' && <div className="notice">{data.message}</div>}
    {chart.length > 0 && <>
      <div className="chartCard"><div className="cardTitle">SARIMA Forecast · {selected} · {months} months</div><ResponsiveContainer width="100%" height={380}><LineChart data={chart}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month" interval={months > 18 ? 2 : 0}/><YAxis/><Tooltip/><Legend/><Line type="monotone" dataKey="forecast_mm" name="Forecast (mm)" stroke="#1976d2" strokeWidth={3} dot={false}/><Line type="monotone" dataKey="lower_95_mm" name="Lower 95%" stroke="#90a4ae" dot={false}/><Line type="monotone" dataKey="upper_95_mm" name="Upper 95%" stroke="#90a4ae" dot={false}/></LineChart></ResponsiveContainer></div>
      <div className="table"><div className="thead"><span>MONTH</span><span>FORECAST</span><span>LOWER 95%</span><span>UPPER 95%</span></div>{chart.map(x => <div className="tr static" key={x.month}><span>{x.month}</span><span>{x.forecast_mm} mm</span><span>{x.lower_95_mm} mm</span><span>{x.upper_95_mm} mm</span></div>)}</div>
    </>}
  </main>
}
...
export default function App() {
  ...
  useEffect(() => { getJson(`${API}/localities`).then(data => { setLocalities(data.localities || []); if (!selected && data.localities?.length) setSelected(data.localities[0].name); }).catch(err => setRiskError(err.message)); }, []);
  ...
  return <div className="app">
    ...
    {page === 'localities' && <LocalitiesPage localities={localities} onOpen={name => { setSelected(name); setPage('map'); }} />}
