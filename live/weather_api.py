...
The live endpoint supports both the existing next-24-hour precipitation
summary and a seven-calendar-day daily precipitation forecast.
"""
...
def get_locality_coords(locality: str) -> dict:
    ...
    if match.empty:
        ...
        raise ValueError(f"Unknown locality '{locality}'. Known localities: {available}")
    row = match.iloc[0]
    return {
...
def daily_forecast(weather_json: dict) -> list[dict]:
    ...
    for i, day in enumerate(dates[:7]):
        ...
        value = r if r is not None else p
        result.append({"date": day, "rainfall_mm": float(value or 0.0)})
    return result
